...
// Edits may only touch a parish-owned DRAFT (unpublished) version. The rule lives in
// core (isEditableDraft); here we just turn a non-editable target into a redirect.
async function assertDraft(parishId: string, versionId: string): Promise<void> {
  if (!(await isEditableDraft(parishId, versionId))) redirect("/ocia/lessons");
}
