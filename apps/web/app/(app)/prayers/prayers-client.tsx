...
export function PrayersClient({ prayers, canEdit }: { prayers: PrayerItem[]; canEdit: boolean }) {
  ...
  const categories = useMemo(
    () => ["all", ...[...new Set(prayers.map((p) => p.category).filter(Boolean) as string[])].sort()],
    [prayers],
  );
          ...
          {filtered.map((p) => (
            <li key={p.id} className="group flex items-start gap-2">
              <button
                ...
                data-testid={`prayer-row-${p.title}`}
              ...
              >
                <span className="flex flex-wrap items-center gap-2">
                  ...
                  {p.category ? (
                    <span className="rounded bg-navy/10 px-1.5 text-xs text-navy capitalize">{p.category}</span>
                  ) : null}
                ...
                <span className="mt-0.5 block truncate text-sm text-gray-500">
                  {normalizeSacredText(p.prayerText).slice(0, 100)}
                </span>
              </button>
              {canEdit ? (
                <span className="flex flex-col gap-1 pt-1 opacity-0 group-hover:opacity-100">
                  ...
                  {p.isLocal ? (
                    <button
                      type="button"
                      onClick={() => startTransition(() => deletePrayerAction(p.id))}
        ...
        >
          ...
          >
            ...
            {selected.overrideNote ? (
              <p className="mt-1 text-xs italic text-gray-400">(parish note) {selected.overrideNote}</p>
            ) : null}
            {selected.latinText ? (
              <p className="mt-3 whitespace-pre-line text-sm italic text-gray-600">{selected.latinText}</p>
            ) : null}
            {selected.context ? (
              <p className="mt-3 text-xs text-gray-500">
                <span className="font-semibold uppercase tracking-wide">When to pray</span>
                <br />
                {normalizeSacredText(selected.context)}
              </p>
            ) : null}
...
function EditModal({ prayer, onClose }: { prayer: PrayerItem | "new"; onClose: () => void }) {
  ...
  function save() {
    ...
    startTransition(async () => {
      if (isNew)
        await addPrayerAction({
          title: f.title,
          prayerText: f.prayerText,
          latinText: f.latinText,
          category: f.category,
          context: f.context,
          attribution: f.attribution,
        });
      else if (isUniversal)
        await overridePrayerAction((prayer as PrayerItem).id, { text: f.prayerText, context: f.context });
      else
        await editPrayerSubmissionAction((prayer as PrayerItem).id, {
          title: f.title,
          prayerText: f.prayerText,
          latinText: f.latinText,
          category: f.category,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div
        ...
        onClick={(e) => e.stopPropagation()}
        data-testid="prayer-edit"
      >
        <h3 className="font-heading text-lg text-navy">{isNew ? "Add prayer" : "Edit prayer"}</h3>
        {isUniversal ? (
          <p className="text-xs text-gray-400">Editing a universal prayer creates a parish override.</p>
        ) : null}
        <input
          className={cls}
          placeholder="Title"
        ...
        <textarea
          ...
          data-testid="prayer-form-text"
        />
        {!isUniversal ? (
          <textarea
            className={cls}
            placeholder="Latin text (optional)"
            rows={2}
            value={f.latinText}
            onChange={set("latinText")}
          />
        ) : null}
        <textarea
          className={cls}
          placeholder="When to pray it (context)"
          rows={2}
          value={f.context}
