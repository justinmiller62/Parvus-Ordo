...
- **Requests and approvals are siloed everywhere.** Join-requests, sign-ups, RSVPs, applications, and approvals each reinvent "pending/approve/assign," and no one has a single place that says *"here's everything waiting on you."*

**The opportunity:** Parvus Gather becomes the single, warm, touch-friendly home for parish involvement — where committees, boards, ministries, and event teams are all variants of one **group**, where it is effortless to **discover, join, organize, meet, schedule, sign up, communicate, and serve**, and where every actionable item across the experience lands in one **Requests** inbox so nothing is missed. It is the "community life" pillar of ParvusOrdo, alongside **formation** (OCIA) and **youth media** (Parvus Studio), and it is built for older volunteers: big touch targets, one primary action per screen, email as the failsafe, and staff who can act on a parishioner's behalf.
...
| **Group admin / leader** | The volunteer (Coordinator, Chair, Grand Knight…) running a committee, board, ministry, or event team | Run their group with minimal admin: roster, roles, meetings, sign-ups, broadcasts, documents — and hand it off cleanly |
| **Board / committee chair & secretary** | Officers of governance groups | Run structured meetings with agenda, quorum, attendance, minutes, per-item discussion — with a durable archive |
...
| **Parish staff / clergy / admin** | The office, the pastor, the parish admin | Toggle and oversee the module, approve pending parishioners, see ministry health, archive zombie groups, **assign and track requests**, and act on any parishioner's behalf |
...
4. Attendance captured; minutes finalized to a **locked PDF**; **action items get assignees + due dates that appear in each person's Requests inbox**.
...
1. After login, the default landing is **"What's next for me?"** — shifts this week, action items, upcoming meetings (RSVP), RSVP'd events, group activity, forms awaiting me.

### 3.10 My Requests — the unified action inbox
> *As anyone in the parish, I want a single place that shows everything assigned to me or waiting on me, so I never miss a request.*
1. I open **Requests** ("you have 3 requests") and see every actionable item across the experience — a join-request to approve, a sign-up to confirm, a meeting RSVP, an action item due, a pending-parishioner approval, a form submission to review.
2. I filter by type, status, or due date; each item shows a **clear next action**.
3. Acting on it (approve / complete / decline / reassign) updates the underlying thing (the roster, the meeting, the application) — I never have to go hunt for where it lives.
4. As a leader/staff, I can **assign** a request to a member and **track** its status to done.

---

## 4. Requirements & Scope

**Full module, no shortcuts.** Requirements mirror the spec's eleven sections (4.1–4.11, numbered to match spec §1–§11 for traceability), plus the embedded Forms Engine (4.12), the roles/permissions model (4.13), older-volunteer design rules (4.14), Non-Goals (4.15), and the delivery sequence (4.16). The net-new **Requests / Requestables** pillar leads as **4.0** because it is cross-cutting and foundational. Tier labels (T1–T8) reflect the human's delivery directive.

### 4.0 Requests / Requestables — the unified action inbox *(cross-cutting, first-class; net-new beyond the base spec)*
A single per-user inbox of actionable, trackable, assignable items. **User-facing name: "Requests." Internal type: "Requestable."**
- Any Gather flow **emits a Request**: a typed item with a payload, an **assignee** (a user — and possibly a role; see §7), a **status lifecycle** (pending → in-progress → done / declined / cancelled), an optional **due date**, a **created-by**, and a link back to its source object.
- Every user gets a **personal Requests inbox/dashboard**: their open items, filterable by type / status / due, each with a clear next action.
- Acting on a Request (approve / complete / decline / reassign) **updates the source object via the owning flow** — Requests handles assignment, tracking, and surfacing; the flow owns the domain action.
- Staff / leaders can **assign** Requests to members and **track** status to completion.
- **Gather flows that feed Requests:** Request-to-Join approvals (4.2), sign-up claims/confirmations (4.5), shift claims & swap requests (4.4), meeting RSVPs and **action items** (4.3), pending-parishioner staff approvals (4.9), and Forms-Engine submissions awaiting review (4.12). The "What's next for me?" dashboard (4.11) and the health dashboard's queues (4.10) are views over this same inbox.
- **Polished UX required:** satisfying approve/complete interactions, clear empty/loading/error states, and respect for reduced-motion preferences.
- **Cross-cutting note (decision deferred to you):** the brief frames Requests as platform-wide infrastructure that other modules (onboarding invites, prayers/dictionary submissions, Parvus Studio approvals, OCIA applications) would also feed. Per direction it is presented here as a first-class **Gather** pillar; whether its underlying engine is built as shared base-platform infrastructure (adopted by those modules) versus Gather-local is **Open Question §7-R1/R2** — not assumed here.
...
- **Action items** (assignee, due date, status) → surfaced in the assignee's Requests inbox.
...
- The full **pending-parishioner** flow (3.7): match-or-create, limited single-group participation, **staff approval queue (in Requests)**, 30/60-day expiry, complete audit.
...
- **My Dashboard / "What's next for me?"** and the **Requests** inbox (4.0) as primary surfaces.
...
- **Reviewer workflow** (approve / reject / request more info) → submissions surface in the reviewer's Requests inbox; **e-signature + PDF**; **CSV/PDF export**; **public embed widget** for the parish website.
...
- **Requests / Requestables (4.0):** net-new, not in the original tier directive. **Recommended foundational** — built with or immediately after T1 — so every later flow emits Requests into one inbox instead of bespoke per-flow queues, and so "What's next for me?" is real from the start. Its exact sequencing (and whether it is shared base-platform infrastructure) is **Open Question §7-R1** — confirm before decomposition.
...
- **Satisfying sign-ups, shifts & requests** — claiming a slot animates and updates a live count; approving/completing a Request feels crisp and final; swaps and releases are just as easy; a friendly 24-hour reminder feels like a thoughtful nudge.
- **Meetings that run themselves** — quorum fills visibly as RSVPs arrive; agenda threads let a board arrive aligned; finalizing minutes closes a chapter cleanly.
...
**Requests effectiveness** — **share of actionable items flowing through the unified Requests inbox** (vs. bespoke queues); **request resolution time** and overdue rate; "nothing missed" (decline-in-stale/abandoned requests); leader/staff use of assign-and-track.
...
*End of PRD-001. This document is `prd-ready` for the user's review and approval. No architect/RFC or decomposition begins until the user marks it `prd-approved`. Once approved, decompose per section along T1–T8 (T1 Groups + Roles/Permissions = P0), with the Requests / Requestables sequencing settled per §7-R1.*
